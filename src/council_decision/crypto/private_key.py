...
class PrivateKey(BaseModel, ABC):
    ...
    @abstractmethod
    def decrypt(self, cipher_text: CipherText, peer_public_key: Union['PublicKey', None] = None) -> PlainText:
        pass
    ...
    def encrypt(self, plain_text: PlainText, _) -> CipherText:
        return self.public_key().encrypt(plain_text, self)
...
class EllipticCurvePrivateKey(PrivateKey):
    key: 'cryptography.EllipticCurvePrivateKey' = Field(default_factory=lambda: ec.generate_private_key(ec.SECP384R1()))

    def shared_cipher(self, peer_public_key: 'EllipticCurvePublicKey', algorithm: BlockCipherAlgorithm = algorithms.AES, mode: modes.Mode = modes.CBC, mode_parameters = None) -> Cipher:
        if mode_parameters is None and mode is modes.CBC:
            mode_parameters = [os.urandom(16)]
        exchange_key = self.key.exchange(algorithm=ec.ECDH(), peer_public_key=peer_public_key.key)
    ...
    def decrypt(self, cipher_text: CipherText, peer_public_key: Union['EllipticCurvePublicKey', None] = None) -> PlainText:
        peer_public_key = peer_public_key if peer_public_key is not None else self.public_key()
        if peer_public_key is None:
            raise KeyError('No peer public key given to calculate shared cipher!')
        decryptor = self.shared_cipher(peer_public_key, mode_parameters=[cipher_text.info['iv']]).decryptor()
