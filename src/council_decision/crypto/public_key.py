...
class PublicKey(BaseModel, ABC):
    ...
    @abstractmethod
    def encrypt(self, plain_text: PlainText, own_private_key: Union['PrivateKey', None] = None) -> CipherText:
        pass
...
class EllipticCurvePublicKey(PublicKey):
    ...
    def encrypt(self, plain_text: PlainText, own_private_key: Union['EllipticCurvePrivateKey', None] = None) -> CipherText:
        own_private_key = own_private_key if own_private_key is not None else self.private_key
        if own_private_key is None:
            raise KeyError('No own private key given for calculating shared communication key!')
        ...
        return CipherText(data=encrypted_data, info={'iv': shared_cipher.mode.initialization_vector})
